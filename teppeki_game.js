tango = [
	...
	['1537', 'come in handly', '便利である、役に立つ'],
...
function make() {
	...
	if (l < 1 || r > tango.length || l > r) {
		...
		document.getElementById("r").value = "2176";
		l = 1;
		r = tango.length;
	}
	var ans_n = Math.floor(Math.random() * (r - l + 1)) + l - 1;
	var quiz = tango[ans_n];
	var c = [];
	c.push(ans_n);
	while (c.length < C_SIZE){
		var k = Math.floor(Math.random() * (r - l + 1)) + l - 1;
		if (c.indexOf(k) === -1){
			c.push(k);
		}
	}
	for (var i = C_SIZE; 1 < i; i--) {
		k = Math.floor(Math.random() * i);
		[c[k], c[i - 1]] = [c[i - 1], c[k]];
	}
	for (var i = 1; i <= C_SIZE; i++){
		document.getElementById("ans" + i).innerText = tango[c[i - 1]][1];
	}
	number = quiz[0];
	english = quiz[1];
	japanese = quiz[2];
